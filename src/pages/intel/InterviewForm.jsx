...
function InterviewFormBody({ draftKey, onClear }) {
  ...
  const handleSubmit = async (e) => {
    ...
    // record_uid = idempotency key ของ interview_save (SEC-12) — สร้างครั้งเดียวต่อฟอร์ม 1 ใบ
    // กดบันทึกซ้ำหลัง error (เช่น เน็ตหลุดหลังฐานข้อมูลบันทึกไปแล้ว) ใช้ uid เดิม → ไม่ได้แถวซ้ำ
    // ใช้ UUID สุ่ม — hash 32 บิตจากข้อมูลฟอร์มแบบเดิมชนกันได้
    if (!recordUidRef.current) {
      recordUidRef.current = 'u:' + (crypto.randomUUID?.() ?? `${Date.now().toString(36)}-${Math.random().toString(36).slice(2)}`)
    }
    ...
    // บันทึก 2 ตารางในธุรกรรมเดียวผ่าน RPC interview_save — ถ้า PII ล้ม เนื้อหาแบบฟอร์มก็ไม่ถูกบันทึกค้าง (SEC-12)
    // audit 'create' บันทึกฝั่งเซิร์ฟเวอร์ในธุรกรรมเดียวกัน (ไม่ต้อง logAction ฝั่งหน้าเว็บ)
    // คืนค่าเป็น jsonb ก้อนเดียว { record_uid, code, doc_no } — ไม่ต้อง .single()
    // ล้มเหลว → ข้อมูลในฟอร์มยังอยู่ครบ กดบันทึกใหม่ได้ด้วย record_uid เดิม
