...
| `20260922a_encrypt.sql` | **ยังไม่ได้รัน — รันก่อนเพื่อน** | หน้าแบบสัมภาษณ์ใช้งานไม่ได้ ดูข้างล่าง |
| `20260922b_drop_plaintext.sql` | ยังไม่ได้รัน — รันหลังทดสอบเฟส 1 ผ่าน | PII ยังเป็น plaintext ติดไปกับ dump/replica |
| `20260922_default_privileges_lockdown.sql` | ยังไม่ได้รัน | ตารางที่สร้างใหม่ยังได้สิทธิ์ anon อัตโนมัติ |

> 🔴 **หน้าแบบสัมภาษณ์พังอยู่ตอนนี้** — การ `revoke` ที่ทำไปเมื่อ 22 ก.ย. ถอนสิทธิ์ตารางออกจาก `authenticated` ด้วย แต่ RPC ชุดเดิมเป็น `security invoker` คือรันด้วยสิทธิ์ผู้เรียก RPC ห้าตัวที่แตะ PII (`interview_save` / `search` / `get` / `export` / `delete`) จึง error `permission denied` ที่ยังไม่มีใครแจ้งเพราะ audit แสดงว่าไม่มีใครเปิดหน้านี้เลยตั้งแต่ revoke
>
> `20260922a_encrypt.sql` แก้ให้แล้วโดยเปลี่ยนเป็น `security definer` (มี `is_admin()` กันอยู่ครบทุกตัว) **จึงต้องรันไฟล์นี้ก่อนไฟล์อื่น**

**ลำดับที่ต้องรัน** — งานเข้ารหัสแบ่งสองเฟสเพราะโปรเจกต์อยู่บน free plan ไม่มี backup อัตโนมัติ

1. `20260922a_encrypt.sql` — เข้ารหัสลง `enc` + เปลี่ยน RPC เป็น definer **ไม่ลบ plaintext** ถ้าพลาดยังกู้ได้
2. ตรวจข้อ 4.1–4.4 ท้ายไฟล์ a ให้ได้ผลว่างครบ + ทดสอบผ่านหน้าเว็บห้าอย่าง
3. `20260922b_drop_plaintext.sql` — มีด่านตรวจสามชั้นในตัว ถ้าไม่ผ่านจะ `raise exception` แล้วยกเลิกทั้ง transaction ไม่ drop อะไรเลย **ขั้นนี้ย้อนไม่ได้**
4. `20260922_default_privileges_lockdown.sql` — ปิดต้นเหตุ รันแยกเมื่อไหร่ก็ได้

ระหว่างอยู่เฟส 1 คอลัมน์ plaintext จะหยุดนิ่งเป็นภาพ ณ เวลาที่รัน ไม่อัปเดตตามการแก้ไขอีก จึงไม่ควรทิ้งช่วงยาวระหว่างเฟส 1 กับเฟส 2
