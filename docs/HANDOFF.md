...
> **อ่านข้อ 3 ก่อน** — การตรวจสิทธิ์กับฐานจริงเมื่อ 22 กันยายน 2569 พบว่าตาราง `interview_records_pii` เปิดให้ role `anon` เข้าถึงได้ ปิดช่องแล้วแต่**ยังไม่ได้ตรวจว่ามีใครเข้าถึงไปจริงหรือไม่** ซึ่งเป็นงานค้างที่ต้องทำก่อนอย่างอื่น
...
| `interview_records_pii` | ชุดเดียวกับข้างบน ผูกกับ `interview_records.record_uid` | RLS + policy แอดมิน + `revoke all` จาก anon/authenticated (**เคยเปิดให้ anon เข้าถึงได้ แก้แล้ว 22 ก.ย. 2569 ดูข้อ 3**) |
...
**สิ่งที่ต้องทำต่อ ยังไม่ได้ทำ**

1. **ตรวจ `audit_logs` และ log ฝั่ง Supabase** ว่ามีใครเคยอ่านตารางนี้ไปจริงหรือไม่ ก่อนสรุปว่าไม่มีข้อมูลรั่ว — ตอนนี้รู้แค่ว่า*ช่องเปิดอยู่* ยังไม่รู้ว่า*มีคนเข้าหรือเปล่า*
...
3. **เพิ่มการตรวจนี้เป็นงานประจำ** รัน SQL ชุดที่ 3 ทุกครั้งหลัง deploy หรือหลังแก้สิทธิ์ผ่าน Dashboard
4. **ประเมินว่าต้องแจ้งเหตุตาม PDPA หรือไม่** ขึ้นกับผลข้อ 1 — ถ้าพบว่ามีการเข้าถึงจริง ต้องแจ้งเจ้าของข้อมูลและหน่วยงานตามกำหนดเวลา
...
- **ตรวจว่ามีใครเข้าถึง `interview_records_pii` ไปจริงหรือไม่** ช่องปิดแล้วแต่ยังไม่รู้ว่าเคยมีคนเข้าหรือเปล่า
